...
class MeltBatch(Base):
    ...
    lot_id           = Column(Integer, nullable=True)  # gold_lots FK removed (no model yet)
    worker_id        = Column(Integer, ForeignKey("workers.id"), nullable=False)
    purity_value     = Column(Float, nullable=True)
    weight_in_g      = Column(Float, nullable=True)   # legacy column kept for DB compat
    input_weight_g   = Column(Float, nullable=True)
    metal_a_g        = Column(Float, default=0.0)
    metal_b_g        = Column(Float, default=0.0)
    extra_alloy_g    = Column(Float, default=0.0)
    total_alloy_g    = Column(Float, default=0.0)
    gross_weight_g   = Column(Float)
    weight_out_916_g = Column(Float)
    ng_weight_g      = Column(Float, default=0.0)
    kambi_weight_g   = Column(Float, default=0.0)
    loss_g           = Column(Float, default=0.0)
    final_916_g      = Column(Float, nullable=False)
    product_type_id  = Column(Integer, ForeignKey("product_types.id"))
    daybook_sno      = Column(Integer, ForeignKey("daybook_entries.serial_no"))
    supplier_id      = Column(Integer, ForeignKey("dealers.id"), nullable=True)
    notes            = Column(Text)
    created_at       = Column(DateTime, server_default=func.now())

    worker           = relationship("Worker")
    supplier         = relationship("Dealer", foreign_keys=[supplier_id])
